...
const AMAP_GEOCODED_INFORMATION_API_URL = 'https://restapi.amap.com/v3/geocode/geo';
const LOCATIONS_API_URL = 'https://openapi.lddgo.net/base/gservice/api/v1/GetIpAddress';
...
async function fetchGeocodedInformation(params) {
	try {
		...
		const response = await axios.get(`${AMAP_GEOCODED_INFORMATION_API_URL}?key=${key}&address=${address}&city=${city}`);
		return response?.data;
	} catch (error) {
	...
	return new Promise((resolve, _reject) => {
		...
		try {
			...
			if (decoded.exp < Math.floor(Date.now() / 1000)) {
				console.error(chalk.red('Error: Token has expired'));
				res.status(401).json({
					code: 401,
